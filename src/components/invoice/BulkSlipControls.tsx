...
interface BulkSlipControlsProps {
  ordersCount: number;
  autoPrint?: boolean;
}

export default function BulkSlipControls({
  ordersCount,
  autoPrint,
}: BulkSlipControlsProps) {
  useEffect(() => {
    ...
    <div className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 px-4 py-3 shadow-xs print:hidden mb-6">
      <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-black text-gray-900 font-mono">
              Bulk 100×100mm Dispatch Slips ({ordersCount} Orders)
            </h1>
            <span className="text-[10px] text-gray-500 font-mono">
              Ready for thermal label printing
            </span>
          </div>
        </div>
        ...
        >
          ...
          <span>Print All {ordersCount} Slips</span>
        </button>
      </div>
    </div>
  );
