      ...
      <div className="space-y-5 animate-fadeIn">
        ...
        <div className="bg-white rounded-2xl p-5 sm:p-7 border border-slate-200/80 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-[#006d36] font-mono text-[10px] font-black uppercase tracking-wider">
                Order Manager
              </span>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Completed parcels successfully handed over to buyers and confirmed delivered.
            </p>
          </div>
        ...
        <div className="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-xs flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          ...
          <span className="text-xs font-mono font-bold text-slate-500 self-end sm:self-auto">
            Delivered Count: <strong className="text-[#006d36]">{filteredOrders.length}</strong>
          </span>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden">
          {loading ? (
            <div className="py-16 flex flex-col items-center justify-center text-slate-400 gap-2">
              ...
              <p className="text-xs font-mono">Loading delivered parcels...</p>
            </div>
            ...
            <div className="py-14 text-center space-y-2">
              ...
              <h3 className="font-bold text-slate-800 text-sm">No Delivered Orders Yet</h3>
              <p className="text-xs text-slate-500 max-w-xs mx-auto">
                Once dispatched parcels are confirmed delivered, they will appear in this log.
              </p>
            </div>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredOrders.map((ord) => (
                    <tr key={ord.id} className="hover:bg-slate-50/60 transition-colors">
                      ...
                      <td className="py-3 px-4">
                        <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-emerald-100 text-[#006d36] font-mono text-[10px] font-bold">
